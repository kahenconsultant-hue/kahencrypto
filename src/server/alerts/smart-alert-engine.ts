...
function premiumMissingNotice(): SmartAlert | null {
  ...
  return alertBase({
    ...
    causalChain: "نبود داده پریمیوم → غیرفعال شدن enrichment → ادامه تحلیل با داده عمومی و پروکسی با confidence پایین‌تر.",
...
function liquidityProxyAlert(): SmartAlert | null {
  ...
  return alertBase({
    ...
    reasoningFa: "رشد استیبل‌کوین و حجم/مومنتوم بازار از داده‌های عمومی هم‌جهت شده‌اند. چون ETF و ذخایر صرافی کامل نیستند، این هشدار proxy-based است.",
...
function stablecoinPressureAlert(): SmartAlert | null {
  ...
  return alertBase({
    ...
    titleFa: "فشار نقدینگی استیبل‌کوین در داده‌های عمومی دیده می‌شود",
